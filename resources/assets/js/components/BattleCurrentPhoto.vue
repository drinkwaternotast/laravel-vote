...
export default {
    ...
    data() {
        return {
            battleList : [],
            error: {},
            initURL: `/api/photo/current/battles/${this.$route.params.photo_id}`,
            namedRoute: 'currentPhotoBattles',
            loading: true,
            counter: 0,
            pagination: {
                total: 0,
    ...
    methods: {
        voteBattle: function(battle_id, photo_id) {
            ...
            .then((res) => {
                if (res.data.saved) {
                    get(this.activeURL+ `?page=` + this.pagination.current_page)
                    .then((res) => { 
