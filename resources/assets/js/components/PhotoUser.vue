...
<div>
    <div class="loading" v-show="loading"><i class="fa fa-spinner fa-pulse fa-fw"></i>loading...</div>
        <div class="photo__header" v-show="!loading">
            <h3>{{title}} List</h3>
            <!-- remove tag function till you implement tag_id and user search -->
            <div class="search_wrapper">
                ...
                <router-link 
                    ...
                    :to="{ name: 'tagSearchPhotos', params: { tag_id1: tag.tag_id1, tag_id2: tag.tag_id2 } }">
...
export default {
    ...
    data: function() {
        return {
            photoList : [],
            error: {},
            initURL: `/api/photo/${this.$route.params.user_id}`,
            namedRoute: 'photos',
            isProcessing: false,
            title: 'Photo',
            status: 'default',
    ...
    methods: {
        ...
        setTagData: function(mutableValue) {
            this.$route.query.tag = mutableValue
            const tags = Object.keys(this.$route.query.tag).length
            if (tags == 1) {
                this.tag.tag_id1 = this.$route.query.tag[0].id
                this.tag.tag_id2 = 0
            }else if (tags == 2) {
                this.tag.tag_id2 = this.$route.query.tag[1].id
            }else if (tags >= 3) {
                this.tag.tag_id1 = this.$route.query.tag[0].id
                this.tag.tag_id2 = this.$route.query.tag[1].id
            }else if (tags == 0) {
